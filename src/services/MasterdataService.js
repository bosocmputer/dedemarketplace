...
export default {
    ...
    getDocImage(limitPage, page, search , sortField, sortOrder , status) {
       ...
       // console.log(`/documentimage?limit=${limitPage}&page=${page}${q}&sort=${sortField}:${sortOrder}&status=${status}`);
        return instanceApi(true).get(`/documentimage?limit=${limitPage}&page=${page}${q}&sort=${sortField}:${sortOrder}&status=${status}`).then(res => res.data);
    },
